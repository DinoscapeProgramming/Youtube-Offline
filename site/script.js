  ...
  if (document.getElementById("downloadLinkInput").value.split("/")[3]?.startsWith("watch")) {
    ...
    downloadVideo(downloadLink).then((downloadedVideo) => {
      downloadedVideo.on("finish", () => {
        ...
        videoElementFurtherData.style.margin = "0";
        videoElementFurtherData.style.paddingTop = "2px";
        videoElementFurtherData.innerText = (new Intl.NumberFormat()).format(JSON.parse(fs.readFileSync(path.join(path.dirname(path.dirname(__dirname)), "data.json"), "utf8"))[videoId].publishDate).locale(navigator.language).fromNow() + " Aufrufe · " + moment(JSON.parse(fs.readFileSync(path.join(path.dirname(path.dirname(__dirname)), "data.json"), "utf8"))[Object.keys(JSON.parse(fs.readFileSync(path.join(path.dirname(path.dirname(__dirname)), "data.json"), "utf8"))).at(-1)].publishDate).locale(navigator.language).fromNow();
        videoElementAdditionalData.appendChild(videoElementAuthorName);
        videoElementAdditionalData.appendChild(videoElementFurtherData);
        videoElementTextData.appendChild(videoElementTitle);
        videoElementTextData.appendChild(videoElementAdditionalData);
    ...
    let downloadPlaylist = (playlistLink) => {
      ...
      .then(({ nextPageToken, items }) => {
        ...
        if (nextPageToken) downloadPlaylist("https://www.googleapis.com/youtube/v3/playlistItems?playlistId=" + downloadLink.split("/")[3].split("=")[1].split("&")[0] + "&part=snippet,id&key=AIzaSyBLkGw0AM5nNQhAyRQHVI0SNp_5J-TrGCg&pageToken=" + nextPageToken);
      });
    };
    downloadPlaylist("https://www.googleapis.com/youtube/v3/playlistItems?playlistId=" + downloadLink.split("/")[3].split("=")[1].split("&")[0] + "&part=snippet,id&key=AIzaSyBLkGw0AM5nNQhAyRQHVI0SNp_5J-TrGCg");
  };
});
